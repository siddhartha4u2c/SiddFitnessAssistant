...
def _compact_ui_css() -> str:
    """Smaller primary/secondary buttons and tab pills app-wide."""
    return """
<style>
    ...
    /* Profile photo is the only file uploader: hide filename + size line under the widget. */
...
<style>
    ...
    /* Header row: session card (log out, right column only) */
    .main .block-container div[data-testid="stHorizontalBlock"]:first-of-type
        div[data-testid="column"]:last-child {
        background: linear-gradient(165deg, #ecfeff 0%, #f0fdfa 45%, #ffffff 100%);
        border: 1px solid rgba(45, 212, 191, 0.45);
        border-radius: 14px;
        padding: 0.55rem 0.9rem 0.65rem;
        box-shadow: 0 2px 14px rgba(13, 148, 136, 0.1);
        margin-top: 0.15rem;
    }
    .main .block-container div[data-testid="stHorizontalBlock"]:first-of-type
    ...
    .main .block-container div[data-testid="stHorizontalBlock"]:first-of-type
        ...
        box-shadow: 0 1px 3px rgba(15, 118, 110, 0.12) !important;
        ...
        def _render_profile_editor() -> None:
            ...
            with st.form("profile_form"):
                ...
                st.text_input(
                    ...
                    value=login_mail,
                    disabled=True,
                    help="Your account email. To change it, create a new account or contact support.",
                )
            ...
            if save_prof:
                ...
                if _save_errs:
                    ...
                    sync_email = db.normalize_login_email(login_mail) if login_mail else ""
                    db.upsert_profile(
                        uid,
                        {
...
if active:
    ...
    col_login, col_app = st.columns([2, 5], gap="large")
    with col_login:
        ...
        with tab_in:
            ...
            st.markdown(
                ...
                'color:#9ca3af;">Forgot password?</p>',
                unsafe_allow_html=True,
            )
            with st.expander("Email a reset link", expanded=False):
                st.caption(
                    "Uses the email in the field above (sign-in or profile inbox on file)."
                )
                ...
                if st.button("Send reset link", use_container_width=False, key="btn_forgot_send"):
                    ...
                    if not fid:
                        ...
                        if send_error:
                            ...
                            st.info(
                                "If an account matches, a reset would go to the email on file "
                                "(sign-in email or **My profile**). Nothing else is shown for privacy."
                            )
            lp = st.text_input(
                "Password", type="password", key="login_pass", autocomplete="current-password"
            )
            if st.button("Sign in", use_container_width=False, key="btn_signin"):
                found = db.verify_user(lu, lp)
                if found:
        ...
        with tab_reg:
            ...
            if st.button("Create account", use_container_width=False, key="btn_reg"):
                if rp != rp2:
                    st.error("Passwords do not match.")
                else:
                    ok, msg = db.create_user(ru, rp)
                    if ok:
